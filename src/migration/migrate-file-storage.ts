...
export default class MigrateFileStorage {
	async migrateTo2_0_0(app: App, settings: ViewCountPluginSettings_1_2_2) {
		const path = getFilePath(app);
		const exists = await app.vault.adapter.exists(path);
		if (exists) {
			Logger.info("Migrating view count file storage from 1.2.2 to 2.0.0");
